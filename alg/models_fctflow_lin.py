...
#             self.bias.data = -torch.mean(input, dim=[0, 2], keepdim=True)
#             std = torch.std(input, dim=[0, 2], keepdim=True)
#             # print(std)
#             self.scale.data = 1 / (std + 1e-10)
#             self.initialized = True

#     def forward(self, input):
#         self.initialize_parameters(input)
#         logdet = (
#             torch.sum(torch.log(torch.abs(self.scale.squeeze())))
#         )
#         return self.scale * (input + self.bias), logdet
...
class FCTflowblock(nn.Module): # Fully convolutional time Flow Block
    def __init__(self, num_channels, net_type, sfactor, hidden_dim, condition_dim):
        super(FCTflowblock, self).__init__()
        self.num_channels = num_channels
        self.sfactor = sfactor
...
class FCTflow(nn.Module): # Fully convolutional time flow
    def __init__(self, num_blocks ,num_channels, net_type, sfactor, hidden_dim, condition_dim):
        ...
        self.blocks = nn.ModuleList([FCTflowblock(self.num_channels, self.net_type,
