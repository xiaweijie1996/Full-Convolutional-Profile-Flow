...
with open(os.path.join(_parent_path, 'exp/cond_rlp_generation/exp_aus/config_aus.yaml')) as file:
    config = yaml.load(file, Loader=yaml.FullLoader)
        
# define the data loader
data_path = os.path.join(_parent_path, 'data/aus_data_cleaned_annual_train.csv')
# read the data from the second column to the end
np_array_train = pd.read_csv(data_path).iloc[:,3:].values
data_path = os.path.join(_parent_path, 'data/aus_data_cleaned_annual_test.csv')
...
# define the wandb
# wandb.init(project="fcpflow_aus")
# wandb.watch(model)

# train the model
path = os.path.join(_parent_path, 'exp/cond_rlp_generation/exp_aus')
