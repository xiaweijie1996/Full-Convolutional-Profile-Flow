...
_row_peak_indx_max = np.unravel_index(np.argmax(np_array_test[:, 48:]), np_array_test[:, 48:].shape)[0]+6
_sample_num = 100
...
model.load_state_dict(torch.load(os.path.join(_parent_path, 'exp/prediction/uk/FCPFlow/FCPflow_model.pth')))
model.eval()
...
# ------------ load the CWGAN-GP ------------
with open(os.path.join(_parent_path, 'exp/prediction/uk/WGANGP/config_cwgan_pre.yaml')) as file:
    config = yaml.load(file, Loader=yaml.FullLoader)
...
generator.load_state_dict(torch.load(os.path.join(_parent_path, 'exp/prediction/uk/WGANGP/CWGANGP.pth')))
generator.eval()
...
nice.load_state_dict(torch.load(os.path.join(_parent_path, 'exp/prediction/uk/NICE/NICE_model.pth')))
nice.eval()
...
vae.load_state_dict(torch.load(os.path.join(_parent_path, 'exp/prediction/uk/VAE/VAE_model.pth')))
vae.eval()
...
# ------------ plot the data ------------
save_path = os.path.join(_parent_path, 'exp/prediction/uk', 'uk_peak_all.png')
re_data_list = None
