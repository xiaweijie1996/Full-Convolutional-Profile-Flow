...
# read the data
data_path = os.path.join(_parent_path, 'data/train_usa_pred.csv')
np_array_train = pd.read_csv(data_path).iloc[:,1:].values
data_path = os.path.join(_parent_path, 'data/test_usa_pred.csv')
...
with open(os.path.join(_parent_path, 'exp/prediction/usa/FCPFlow/config_pre.yaml')) as file:
...
model.load_state_dict(torch.load(os.path.join(_parent_path, 'exp/prediction/usa/FCPFlow/FCPflow_model.pth')))
model.eval()
...
# ------------ load the CWGAN-GP ------------
with open(os.path.join(_parent_path, 'exp/prediction/usa/WGANGP/config_cwgan_pre.yaml')) as file:
    config = yaml.load(file, Loader=yaml.FullLoader)
...
generator.load_state_dict(torch.load(os.path.join(_parent_path, 'exp/prediction/usa/WGANGP/CWGANGP.pth')))
generator.eval()
...
# ------------ load the NICE ------------
with open(os.path.join(_parent_path, 'exp/prediction/usa/NICE/config_nice_pre.yaml')) as file:
    config = yaml.load(file, Loader=yaml.FullLoader)
...
nice.load_state_dict(torch.load(os.path.join(_parent_path, 'exp/prediction/usa/NICE/NICE_model.pth')))
nice.eval()
...
# ------------ load the VAE ------------
with open(os.path.join(_parent_path, 'exp/prediction/usa/VAE/config_vae_pre.yaml')) as file:
    config = yaml.load(file, Loader=yaml.FullLoader)
...
vae.load_state_dict(torch.load(os.path.join(_parent_path, 'exp/prediction/usa/VAE/VAE_model.pth')))
vae.eval()
...
# plot all the data
save_path = os.path.join(_parent_path, 'exp/prediction/usa', 'usa_all.png')
plt.figure(figsize=(20,10))
plt.plot(all_data[:,96:].T, color='blue', alpha=0.1)
